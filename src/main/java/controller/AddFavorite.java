	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (userInfoOnSession != null) {
			...
			//パラメータのバリデーションチェック
				...
				//リクエストパラメータを取得
				int userId = userInfoOnSession.getUserId(); //リクエストパラメータ（userId）
				int itemId = id; //リクエストパラメータ（itemId）
				...
				AddFavoriteBL logic = new AddFavoriteBL();
				flugcheck = logic.executeInsertFavorite(dto); //成功フラグ（true:成功/false:失敗）
			...
			if (flugcheck ==  0) {
				...
				String url ="view/cartin.jsp?id=" + itemId;
				...
				response.sendRedirect("htmls/error2.html");
			}
